...
class LinearElasticityFEniCSxExecuter(executers.BaseExecuter):
    ...
    def pre_process(self):
        ...
        # Check if a custom mesh is provided
        if hasattr(self.args, "mesh_filename") and self.args.mesh_filename:
            mesh_path = os.path.join(sim_dir_path, self.args.mesh_filename)
            os.rename(mesh_path, os.path.join(sim_dir_path, MESH_FILENAME))

            # If mesh info filename is provided
            if hasattr(self.args,
                       "mesh_info_filename") and self.args.mesh_info_filename:
                mesh_info_path = os.path.join(sim_dir_path,
                                              self.args.mesh_info_filename)
                shutil.copy(
                    mesh_info_path,
                    os.path.join(self.artifacts_dir, MESH_INFO_FILENAME))
            ...
            mesh = mesh_utils.GmshMesh(
                geometry=geometry,
                global_refinement_factor=self.args.
                global_refinement_meshing_factor,
                local_refinement_factor=self.args.
            ...
            mesh_info_path = os.path.join(self.artifacts_dir,
                                          MESH_INFO_FILENAME)
