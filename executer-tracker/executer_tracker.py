...
def monitor_redis_stream(redis_connection, stream_name, last_id=0):
    """
    ...
    while True:
        try:
            ...
            if resp:
                ...
                # Right now, this avoids reprocessing the same request twice,
                # but maybe it would be better
                # to find a solution to persist "last_id" accross
                # executions of a given worker
                request_status = redis_connection.get(
                    make_task_key(last_id, "status"))
                logging.info("Status of request %s: %s", last_id,
                             request_status)
                ...
                redis_connection.set(make_task_key(last_id, "status"),
                                     "started")
                ...
                for key, value in output_dict.items():
                    if key.endswith("_path"):
                        ...
                        dst_dir = os.path.join(artifact_dest, last_id)
                ...
                redis_connection.hset(make_task_key(last_id, "output"),
                                      mapping=output_dict)
                logging.info("Stored outputs of job with ID %s in Redis",
                             last_id)

                # Mark task as finished successfully
                out = redis_connection.set(make_task_key(last_id, "status"),
                                           "success")
