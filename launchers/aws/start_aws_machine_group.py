...
parser = argparse.ArgumentParser(
    description="Launch Virtual Machines (VMs) by configuring them.")
parser.add_argument(
    "--vm_type",
    type=str,
    default="t2.micro",
    help=
    "Virtual Machine Type [default: t2.micro]. Learn more at https://aws.amazon.com/pt/ec2/instance-types/",
)
parser.add_argument(
    "--volume_size",
    type=int,
...
parser.add_argument(
    ...
    type=int,
    default=3000,
    help="IOPS for the volume (applies to io1, io2, gp3 only) [default: 3000]",
)
parser.add_argument(
    "--throughput",
    type=int,
...
# Create a key pair (pem or ppk) for SSH access to the Virtual Machine
...
instance_profile_name = create_iam_role_with_admin_access(
    role_name, args.profile)

session = boto3.Session(profile_name=args.profile, region_name=args.region)
ec2_client = session.client("ec2")

with open(args.user_data_path, encoding="UTF8") as f:
    template = Template(f.read())

user_data_rendered = template.substitute(
    INDUCTIVA_API_KEY=args.api_key,
    MACHINE_GROUP_NAME=args.machine_group_name,
)

for i in range(args.num_machines):
    unique_name = (
        f"VM-{i + 1}-{datetime.datetime.now().strftime('%Y_%m_%d_%H_%M')}")
    try:
        response = ec2_client.run_instances(
            ...
            BlockDeviceMappings=[{
                ...
                "Ebs": {
                    ...
                    "Iops":
                        args.iops
                        if args.volume_type in ["gp3", "io1", "io2"] else None,
                    "Throughput":
                        args.throughput if args.volume_type == "gp3" else None,
                },
            }],
        )
